...
class Nertz_Url_Chpu extends Nertz_Url_Handler
{
	/**
	 * Конструктор
	 * 
	 */
	function Nertz_Url_Chpu()
	{
		...
		// Разберем что нам нагадал урль реврайт, 
		$u = parse_url($_SERVER['REQUEST_URI']);
		if (!empty($this->prefix) && strpos($u['path'],$this->prefix) === 0) {
			$u['path'] = substr($u['path'],strlen($this->prefix) -1);
		}
		$arr = explode('/', $u['path']);
		array_shift($arr);
		array_pop($arr);
		...
		if (count($arr)) {
			...
			$res['page'] = $page;
			$_REQUEST['page'] = $page;
			$params = $this->get_page_params($page);
			if (count($params)) {
				$i = 0;
	...
	{
		$post = 0;
		if (isset( $_POST[$this->gen_post_name( $name )])) {
			$post = 1;
		}
	...
	function gen_url($params)
	{
		...
		$pp = $this->get_page_params($page);
		unset($params['page']);
		$path = trim($this->prefix,"/") .'/' . $page . '/';
		if (count($pp)) {
			$res =  array();
			$i = 0;
			foreach ($pp as $name => $p) {
				$res[$i] = !empty($params[$name]) ? $params[$name] : $p['default']; 	
				unset($params[$name]);
				$i++;
			}
			 $path .= implode('/', $res) . '/';
		...
		$path = rtrim($path, '/') . '/';
		if ($path == '/' . Nertz::config('/main/default_page') . '/') {
			$path = '/';
		}
		$url = "?";
		$amp = "";
		...
		if ($full_url || $custom_server_name) {
			return  'http://' . ($custom_server_name ? $custom_server_name : Nertz::server_name()) . '' . $path . $url;
		}
		if ($encode) {
			return '/' . $path. urlencode($url);
		}
		return  $path . $url;
	}
	/**
